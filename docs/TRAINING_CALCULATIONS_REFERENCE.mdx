...
const maxHeartRate =
  maxHRs.length > 0 ? maxHRs[Math.floor(maxHRs.length * 0.05)] || 190 : 190; // Default fallback
```
...
const restingHeartRate =
  avgHRs.length > 0 ? avgHRs[Math.floor(avgHRs.length * 0.05)] || 60 : 60; // Default fallback
```
...
const zones = {
  zone_1: {
    min: restHR,
    max: restHR + hrReserve * 0.6,
    name: 'Active Recovery',
  },
  zone_2: {
    min: restHR + hrReserve * 0.6,
    max: restHR + hrReserve * 0.7,
    name: 'Aerobic Base',
  },
  zone_3: {
    min: restHR + hrReserve * 0.7,
    max: restHR + hrReserve * 0.8,
    name: 'Aerobic Threshold',
  },
  zone_4: {
    min: restHR + hrReserve * 0.8,
    max: restHR + hrReserve * 0.9,
...
const zones = {
  ...
  zone_2: {
    min: thresholdPace * 1.1,
    max: thresholdPace * 1.3,
    name: 'Aerobic',
  },
  zone_3: {
    min: thresholdPace * 1.0,
    max: thresholdPace * 1.1,
    name: 'Threshold',
  },
  zone_4: {
    min: thresholdPace * 0.95,
    max: thresholdPace * 1.0,
    name: 'Lactate Threshold',
  },
  zone_5: {
...
_This document serves as the definitive reference for all training calculations and metrics in the EnduroRevamp application. For implementation details, refer to the source code files listed in each section._
