...
describe('PersonalBests', () => {
  ...
  it('renders empty state when no activities provided', () => {
    ...
    expect(
      screen.getByText('No activities found to calculate race bests')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Sync your activities from Strava to see your records')
    ).toBeInTheDocument();
  });
  ...
  it('renders race bests when activities are provided', () => {
    ...
    expect(screen.getByText('Race Bests')).toBeInTheDocument();
    expect(
      screen.getByText('Your best times at common race distances')
    ).toBeInTheDocument();
  });
  ...
  it('shows empty state when no running activities', () => {
    ...
    expect(
      screen.getByText('No race distance activities found')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Complete runs at 1 mile, 5K, 10K, half marathon, or marathon distances/
      )
    ).toBeInTheDocument();
  });
  ...
  it('sorts distances from shortest to longest', () => {
    ...
    const oneMileCard = raceCards.find(card =>
      card.textContent?.includes('1 Mile')
    );
