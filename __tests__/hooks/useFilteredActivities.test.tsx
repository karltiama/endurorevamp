...
describe('useFilteredActivities', () => {
  ...
  it('should return all activities when filter is "all"', () => {
    mockUseUserActivities.mockReturnValue(
      createMockQueryResult(mockActivities)
    );

    const { result } = renderHook(() =>
      useFilteredActivities('user1', 'all', 'date-desc')
    );
  ...
  it('should filter activities by type "run"', () => {
    ...
    const { result } = renderHook(() =>
      useFilteredActivities('user1', 'run', 'date-desc')
    );

    expect(result.current.activities).toHaveLength(2);
    expect(
      result.current.activities.every(activity => activity.sport_type === 'Run')
    ).toBe(true);
    expect(result.current.totalCount).toBe(3);
  ...
  it('should filter activities by type "ride"', () => {
    ...
    const { result } = renderHook(() =>
      useFilteredActivities('user1', 'ride', 'date-desc')
    );
  ...
  it('should return empty array for favorite filter when no favorites exist', () => {
    const { result } = renderHook(() =>
      useFilteredActivities('user1', 'favorite', 'date-desc')
    );
  ...
  it('should return only favorite activities when favorite filter is applied', () => {
    ...
    expect(result.current.activities).toHaveLength(2);
    expect(
      result.current.activities.every(activity => activity.is_favorite)
    ).toBe(true);
    expect(result.current.filteredCount).toBe(2);
  });
  ...
  describe('sorting functionality', () => {
    ...
    it('should sort activities by date ascending (oldest first)', () => {
      ...
      const { result } = renderHook(() =>
        useFilteredActivities('user1', 'all', 'date-asc')
      );
    ...
    it('should sort activities by distance descending (longest first)', () => {
      ...
      const { result } = renderHook(() =>
        useFilteredActivities('user1', 'all', 'distance-desc')
      );
    ...
    it('should sort activities by name ascending (A-Z)', () => {
      ...
      const { result } = renderHook(() =>
        useFilteredActivities('user1', 'all', 'name-asc')
      );
