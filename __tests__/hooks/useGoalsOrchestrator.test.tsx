...
describe('useGoalsOrchestrator', () => {
  ...
  it('should provide analytics and recommendations', async () => {
    ...
    mockOrchestrator.getGoalRecommendations.mockResolvedValue(mockRecommendations);
    ...
    expect(mockOrchestrator.getGoalRecommendations).toHaveBeenCalledWith('user-1');
  });
  ...
  it('should provide goal creation functions', async () => {
    ...
    expect(mockOrchestrator.createGoal).toHaveBeenCalledWith(goalData, { type: 'manual' });
  });
  ...
  it('should provide goal update functions', async () => {
    ...
    expect(mockOrchestrator.updateGoal).toHaveBeenCalledWith('goal-1', updates, context);
  });

  it('should provide dashboard management functions', async () => {
    const mockUpdatedGoals = [
      { id: 'goal-1', goal_data: { show_on_dashboard: true, dashboard_priority: 1 } },
      { id: 'goal-2', goal_data: { show_on_dashboard: true, dashboard_priority: 2 } }
    ] as any;
    ...
    const managedGoals = await result.current.manageDashboardGoals(['goal-1', 'goal-2']);

    expect(managedGoals).toEqual(mockUpdatedGoals);
    expect(mockOrchestrator.manageDashboardGoals).toHaveBeenCalledWith(['goal-1', 'goal-2'], 'user-1');
  });
  ...
  it('should provide bulk update functions', async () => {
    ...
    const updatedGoals = await result.current.bulkUpdateGoals({ updates, context });

    expect(updatedGoals).toEqual(mockUpdatedGoals);
    expect(mockOrchestrator.bulkUpdateGoals).toHaveBeenCalledWith(updates, context);
  });
  ...
  it('should provide archive function', async () => {
    ...
    expect(mockOrchestrator.archiveCompletedGoals).toHaveBeenCalledWith('user-1');
  });
