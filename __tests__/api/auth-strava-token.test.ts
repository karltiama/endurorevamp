...
describe('/api/auth/strava/token', () => {
  ...
  describe('POST /api/auth/strava/token', () => {
    ...
    it('should successfully exchange code for tokens', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith('https://www.strava.com/oauth/token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      expect(mockCookieStore.set).toHaveBeenCalledWith('strava_connected', 'true', {
        httpOnly: true,
        secure: false,
        sameSite: 'lax',
        maxAge: 60 * 60 * 24
    ...
    it('should return error when Strava API returns error', async () => {
      ...
      ;(global.fetch as jest.Mock).mockResolvedValue({
        ok: false,
        status: 400,
        text: jest.fn().mockResolvedValue(JSON.stringify(mockStravaErrorResponse))
      })
      ...
      expect(response.status).toBe(400)
      expect(data).toEqual({
        error: 'Strava API error: 400 - {"message":"invalid_grant","errors":[{"resource":"AuthorizationCode","field":"code","code":"invalid"}]}'
      })
    })
    ...
    it('should return 500 when token storage fails', async () => {
      ...
      expect(data).toEqual({ error: 'Failed to store Strava tokens in database' })
    })
