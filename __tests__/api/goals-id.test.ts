...
describe('Goals [id] API', () => {
  ...
  describe('PATCH /api/goals/[id]', () => {
    ...
    it('should return 404 if goal not found', async () => {
      ...
      mockGetUser.mockResolvedValue(mockUser)
      const request = new Request('http://localhost/api/goals/goal-1', { method: 'PATCH', body: '{}' })
      const mockSupabaseClient = {
        from: jest.fn().mockReturnValue({
          select: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
      ...
      const mockSupabaseClient = {
        from: jest.fn().mockReturnValue({
          select: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
              eq: jest.fn().mockReturnValue({
                single: jest.fn().mockResolvedValue({ data: { id: 'goal-1' }, error: null })
              })
            })
          }),
          update: jest.fn().mockReturnValue({
  ...
  describe('DELETE /api/goals/[id]', () => {
    ...
    it('should delete a goal successfully', async () => {
      ...
      mockGetUser.mockResolvedValue(mockUser)
      const request = new Request('http://localhost/api/goals/goal-1', { method: 'DELETE' })
      const mockSupabaseClient = {
        from: jest.fn().mockReturnValue({
          select: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
      ...
      const data = await response.json()
      expect(response.status).toBe(200)
      expect(data).toEqual({ success: true, message: 'Goal deleted successfully' })
    })
