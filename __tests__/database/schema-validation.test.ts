  ...
  describe('Data Flow Validation', () => {
    it('complete user goal flow should be queryable', async () => {
      ...
      if (data && data.length > 0) {
        ...
        // Should have goal type information
        expect(goal.goal_types).toBeTruthy()
        expect(goal.goal_types.display_name).toBeTruthy()
        
        // Progress might be empty for new goals, that's ok
        expect(Array.isArray(goal.goal_progress)).toBe(true)
      }
    })
  })
})
