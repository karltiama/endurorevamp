...
describe('DynamicGoalEngine', () => {
  ...
  describe('generateDynamicSuggestions', () => {
    ...
    it('should map goal categories to correct goal types', () => {
      ...
      const distanceSuggestions = suggestions.filter(s => s.category === 'distance')
      ...
      const frequencySuggestions = suggestions.filter(s => s.category === 'frequency')
    ...
    it('should convert distance goals to user preferred units', () => {
      ...
      const kmSuggestions = DynamicGoalEngine.generateDynamicSuggestions(profile, mockGoals, { distance: 'km', pace: 'min/km' })
      const kmDistanceSuggestions = kmSuggestions.filter(s => s.category === 'distance')
      ...
      // Test with miles
      const mileSuggestions = DynamicGoalEngine.generateDynamicSuggestions(profile, mockGoals, { distance: 'miles', pace: 'min/mile' })
      const mileDistanceSuggestions = mileSuggestions.filter(s => s.category === 'distance')
    ...
    it('should convert target values appropriately for different units', () => {
      ...
      const mileDistanceGoal = mileSuggestions.find(s => s.category === 'distance')
      
      if (kmDistanceGoal && mileDistanceGoal) {
        // The mile target should be approximately 0.621371 times the km target
        const expectedMileTarget = Math.round(kmDistanceGoal.suggestedTarget * 0.621371 * 10) / 10
        expect(mileDistanceGoal.suggestedTarget).toBeCloseTo(expectedMileTarget, 1)
      }
    })
    ...
    it('should have valid goal types for specific problematic suggestions', () => {
      ...
      const nextStepSuggestion = suggestions.find(s => s.id === 'dynamic-beginner-progression')
      ...
      const challengeSuggestion = suggestions.find(s => s.id === 'dynamic-10k-challenge')
      ...
      const recoverySuggestion = suggestions.find(s => s.id === 'dynamic-frequency-recovery')
