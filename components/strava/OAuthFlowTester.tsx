...
export function OAuthFlowTester() {
  ...
  useEffect(() => {
    ...
    if (state === 'oauth-test') {
      if (code) {
        console.log(
          '✅ OAuth test callback received with code:',
          code.substring(0, 12) + '...'
        );
        setFlowState(prev => ({
          step: 4,
          steps: prev.steps.map((step, index) => ({
            ...
            details:
              index === 3
                ? `Received code: ${code.substring(0, 12)}...`
                : step.details,
          })),
        }));
        ...
        setTimeout(() => {
          ...
          setFlowState(prev => ({
            ...
            steps: prev.steps.map((step, index) => ({
              ...step,
              status:
                index < 5 ? 'complete' : index === 5 ? 'active' : 'pending',
              details:
                index === 4 ? 'Exchanging code for tokens...' : step.details,
            })),
          }));
        ...
        console.error(
          '❌ OAuth test callback received error:',
          error,
          errorDescription
        );
        setFlowState(prev => ({
  ...
  const testTokenExchange = async (code: string) => {
    try {
      console.log(
        '🔄 Testing token exchange with code:',
        code.substring(0, 12) + '...'
      );
      ...
      setFlowState(prev => ({
        ...
        steps: prev.steps.map((step, index) => ({
          ...
          details:
            index === 4
              ? `Tokens received for athlete: ${result.athlete?.firstname} ${result.athlete?.lastname}`
              : index === 5
    ...
    setTimeout(() => {
      ...
      setFlowState(prev => ({
        ...
        steps: prev.steps.map((step, index) => ({
          ...
          details:
            index === 0 ? `URL: ${authUrl.substring(0, 50)}...` : step.details,
        })),
      }));
            ...
            <>
              <div className="flex-1">
                <Alert>
                  ...
                  <AlertDescription>
                    OAuth flow in progress. Check browser console for detailed
                    logs.
                    {flowState.step === 3 && (
                      <div className="mt-2">
                        <strong>Next:</strong> User needs to authorize on
                        Strava, then they&apos;ll be redirected back to this
                        test page.
                      </div>
                    )}
                    {flowState.step >= 6 && (
                      <div className="mt-2">
                        <strong>✅ Test Complete!</strong> OAuth flow worked
                        successfully.
                      </div>
                    )}
                  </AlertDescription>
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <ol className="mt-2 list-decimal list-inside space-y-1 text-sm">
              <li>
                Generates a test OAuth URL that redirects back to this page
              </li>
              <li>Redirects you to Strava for authorization</li>
              <li>
                Strava redirects back to this test page with authorization code
              </li>
              <li>Tests the token exchange API call with the received code</li>
              <li>
                Shows you the complete flow without interrupting your testing
              </li>
              <li>
                <strong>Note:</strong> This stores real tokens in your database!
              </li>
            </ol>
            <div className="mt-3 p-2 bg-yellow-50 rounded text-sm">
              <strong>⚠️ Important:</strong> Make sure your Strava app&apos;s
              &quot;Authorization Callback Domain&quot; includes{' '}
              <code className="bg-white px-1 rounded">localhost</code> for this
              test to work.
            </div>
          </AlertDescription>
        </Alert>
