...
import { formatDistance, getActivityIcon, formatStravaDate, formatStravaTime, formatPace } from '@/lib/utils'
...
export function ActivityCard({ activity, onViewDetails }: ActivityCardProps) {
  ...
  const getActivityMetrics = () => {
    ...
    if (normalized.average_speed && ['Ride', 'Hike', 'Swim'].includes(normalized.type)) {
      metrics.push({
        label: 'Avg Speed',
    ...
    if (normalized.total_elevation_gain > 0 && !['Run', 'Walk'].includes(normalized.type)) {
      metrics.push({
        label: 'Elevation',
  ...
  const getAdditionalMetrics = () => {
    ...
    if (normalized.average_heartrate && ['Ride', 'Hike', 'Swim'].includes(normalized.type)) {
      metrics.push({
        icon: '❤️',
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-lg transition-all duration-200 hover:border-gray-300">
      ...
      <div className="mb-4">
        <div className={`grid gap-4 ${activityMetrics.length === 1 ? 'grid-cols-1' : activityMetrics.length === 2 ? 'grid-cols-2' : activityMetrics.length === 3 ? 'grid-cols-3' : 'grid-cols-2 md:grid-cols-4'}`}>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={normalized.is_favorite ? 'Remove from favorites' : 'Add to favorites'}
          >
            {normalized.is_favorite ? (
