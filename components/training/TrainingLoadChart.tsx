        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="trends" className="space-y-6">
            {chartData.length > 0 ? (
              <div className="space-y-4">
                <div className="h-64">
                  <h4 className="text-sm font-medium mb-2">Training Load Trends (90 days)</h4>
                  <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="h-64">
                  <h4 className="text-sm font-medium mb-2">Daily Training Load</h4>
                  <ResponsiveContainer width="100%" height="100%">
...
function DataQualityBadge({ quality }: { quality: string }) {
  const variants = {
    excellent: { variant: 'default' as const, color: 'bg-green-500', icon: CheckCircle },
    good: { variant: 'secondary' as const, color: 'bg-blue-500', icon: CheckCircle },
    fair: { variant: 'outline' as const, color: 'bg-yellow-500', icon: AlertTriangle },
    poor: { variant: 'outline' as const, color: 'bg-orange-500', icon: AlertTriangle },
    none: { variant: 'outline' as const, color: 'bg-red-500', icon: AlertTriangle }
  }
...
function TrainingLoadMetrics({ metrics }: { metrics: any }) {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'recover': return 'bg-yellow-500'
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'build': return TrendingUp
      case 'maintain': return Minus
      case 'recover': return TrendingDown
        ...
        <div className="space-y-2">
          ...
          <div className={`h-2 rounded-full ${metrics.balance > 0 ? 'bg-green-200' : 'bg-red-200'}`}>
            <div 
              ...
              style={{ width: `${Math.min(100, Math.abs(metrics.balance) * 5)}%` }}
            />
          </div>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="space-y-1">
          <label className="text-xs font-medium text-muted-foreground">Max HR</label>
          <div className="text-lg font-semibold">{thresholds.maxHeartRate} bpm</div>
        </div>
        
        <div className="space-y-1">
          <label className="text-xs font-medium text-muted-foreground">Resting HR</label>
          <div className="text-lg font-semibold">{thresholds.restingHeartRate} bpm</div>
        </div>
        
        {thresholds.functionalThresholdPower && (
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">FTP</label>
            <div className="text-lg font-semibold">{Math.round(thresholds.functionalThresholdPower)}W</div>
          </div>
        )}
        
        {thresholds.lactateThreshold && (
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">LTHR</label>
            <div className="text-lg font-semibold">{Math.round(thresholds.lactateThreshold)} bpm</div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3 text-sm text-muted-foreground">
        <div>
          <span className="font-medium">Fitness (CTL):</span> Your chronic training load over 42 days. Higher values indicate better fitness.
        </div>
        <div>
          <span className="font-medium">Fatigue (ATL):</span> Your acute training load over 7 days. Represents short-term fatigue.
        </div>
        <div>
          <span className="font-medium">Form (TSB):</span> Training Stress Balance = Fitness - Fatigue. Positive values suggest good form for performance.
        </div>
        <div>
          <span className="font-medium">TRIMP:</span> Training Impulse calculated from heart rate and duration with sport-specific adjustments.
        </div>
        <div>
          <span className="font-medium">TSS:</span> Training Stress Score based on power (when available) or heart rate intensity.
        </div>
      </div>
    </div>
  )
