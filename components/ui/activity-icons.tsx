...
import { Activity, Home, Bike, Dumbbell, Mountain, Zap } from 'lucide-react';
...
export function ActivityIcon({
  ...
  if (t === 'workout' || t === 'weighttraining')
    return <Dumbbell className={className} />;
  // Fallbacks for swim, walk, etc.
  if (t === 'swim')
    return (
      <span className={className} role="img" aria-label="Swim">
        🏊‍♂️
      </span>
    );
  if (t === 'walk')
    return (
      <span className={className} role="img" aria-label="Walk">
        🚶‍♂️
      </span>
    );
...
export function SmartTreadmillIcon({
  className = 'h-6 w-6',
}: {
  className?: string;
}) {
