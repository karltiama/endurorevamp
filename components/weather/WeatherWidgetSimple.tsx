...
import { Thermometer, Droplets, Wind, CloudRain, Sun, AlertTriangle, MapPin } from 'lucide-react'
import { useWeather } from '@/hooks/useWeather'

interface WeatherWidgetSimpleProps {
  className?: string
  showImpact?: boolean
  showOptimalTime?: boolean
  // Default to a major city - we can make this configurable later
  defaultLat?: number
  defaultLon?: number
}

export function WeatherWidgetSimple({ 
  className = '', 
  showImpact = true, 
  showOptimalTime = true,
  defaultLat = 51.5074, // London coordinates as default
  defaultLon = -0.1278
}: WeatherWidgetSimpleProps) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="pt-2 border-t">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Showing weather for {location?.name || 'London'}</span>
          </div>
        </div>
      </CardContent>
    </Card>
