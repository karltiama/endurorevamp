...
export function StravaDataAnalyzer() {
  ...
  const getFieldPriority = (
    fieldName: string
  ): { priority: 'high' | 'medium' | 'low'; phase: 1 | 2 | 3 } => {
    ...
    if (mediumPriority.includes(fieldName))
      return { priority: 'medium', phase: 2 };
  ...
  const analyzeStravaData = async () => {
    ...
    try {
      ...
      const analysisResults: FieldAnalysis[] = Array.from(
        fieldStats.entries()
      ).map(([fieldName, stats]) => {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <Button
            onClick={analyzeStravaData}
            size="sm"
            disabled={isAnalyzing || !accessToken || !user}
          >
      ...
      <CardContent>
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            This analyzes your actual Strava data to help design an optimal
            schema based on what fields are available and how they&apos;re used.
          </p>

          {!analysis.length && !isAnalyzing && (
            <div className="text-center py-8 text-gray-500">
              Click &quot;Analyze Strava Data&quot; to examine your activity
              data structure
            </div>
          )}
                      ...
                      {phaseFields.map((field, index) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            <span>
                              {field.frequency}/{rawData.length} activities
                            </span>
                            <Badge
                              variant={
                                field.priority === 'high'
