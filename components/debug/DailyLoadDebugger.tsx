  ...
  const analyzeDates = () => {
    ...
    try {
      ...
      const analyzeDate = (targetDate: string): DailyAnalysis | undefined => {
        ...
        debug.push(
          `Found ${dayActivities.length} activities for ${targetDate}`
        );

        if (dayActivities.length === 0) return undefined;

        const activityAnalyses: ActivityAnalysis[] = dayActivities.map(
          activity => {
            try {
              const trimp = calculator.calculateTRIMP(activity);
              const tss = calculator.calculateTSS(activity);
              const normalizedLoad =
                calculator.calculateNormalizedLoad(activity);
        ...
        const totalTrimp = activityAnalyses.reduce(
          (sum, act) => sum + act.trimp,
          0
        );
        const totalTss = activityAnalyses.reduce(
          (sum, act) => sum + act.tss,
          0
        );
        const totalLoad = activityAnalyses.reduce(
          (sum, act) => sum + act.normalizedLoad,
          0
        );

        debug.push(
          `Date ${targetDate} totals: TRIMP=${totalTrimp}, TSS=${totalTss}, Load=${totalLoad}`
        );
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <Badge variant="outline">
                          {analysis.date1.totalLoad.toFixed(1)}
                        </Badge>
                      </div>
                      <div className="flex justify-between">
                        <span>Total TRIMP:</span>
                        <span className="text-sm">
                          {analysis.date1.totalTrimp.toFixed(1)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total TSS:</span>
                        <span className="text-sm">
                          {analysis.date1.totalTss.toFixed(1)}
                        </span>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        <span>Total Load:</span>
                        <Badge variant="outline">
                          {analysis.date2.totalLoad.toFixed(1)}
                        </Badge>
                      </div>
                      <div className="flex justify-between">
                        <span>Total TRIMP:</span>
                        <span className="text-sm">
                          {analysis.date2.totalTrimp.toFixed(1)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total TSS:</span>
                        <span className="text-sm">
                          {analysis.date2.totalTss.toFixed(1)}
                        </span>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    {analysis.date1.activities.map((activity, index) => (
                      <div key={index} className="border rounded p-3 mb-2">
                        ...
                        <div className="text-xs space-y-1">
                          <div>
                            Duration: {Math.round(activity.duration / 60)}min
                          </div>
                          {activity.avgHR && (
                            <div>Avg HR: {activity.avgHR}</div>
                          )}
                          {activity.avgPower && (
                            <div>Avg Power: {activity.avgPower}W</div>
                          )}
                          <div className="flex justify-between">
                          ...
                          <div className="font-medium">
                            Load: {activity.normalizedLoad.toFixed(1)}
                          </div>
          ...
          {(!analysis.date1 || !analysis.date2) && !error && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm mt-2">
                Make sure you have activities on both dates: {selectedDate1} and{' '}
                {selectedDate2}
              </p>
            </div>
          )}
