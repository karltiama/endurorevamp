...
export function DetailedSyncDebugger() {
  ...
  const debugSpecificActivity = async () => {
    ...
    try {
      ...
      Object.entries(targetActivity).forEach(([key, value]) => {
        if (typeof value === 'string') {
          ...
          if (
            ...
            console.warn(
              `⚠️ Found pace string in raw Strava data field '${key}': "${value}"`
            );
          }
        }
      });
          ...
          if (Array.isArray(value)) {
            value.forEach((item, index) => {
              if (
                ...
                console.warn(
                  `⚠️ Found pace string in array ${key}[${index}]: "${item}"`
                );
              }
            });
          }
      ...
      const problematicFields = Object.entries(targetActivity).filter(
        ([, value]) => {
          return (
            typeof value === 'string' &&
            (value.includes('/km') ||
      ...
      if (problematicFields.length > 0) {
        console.log(
          '⚠️ Found pace strings that need conversion:',
          problematicFields
        );
      }

      const fieldAnalysis = Object.entries(targetActivity).map(
        ([key, value]) => {
          const analysis = {
            field: key,
            type: typeof value,
          ...
          if (typeof value === 'string') {
            ...
            const isPaceString =
              pacePatterns.some(pattern => pattern.test(str)) ||
              (str.includes(':') &&
                (str.includes('/km') ||
                  str.includes('/mi') ||
                  str.includes('/ km') ||
            ...
            if (isPaceString) {
              ...
              console.log(
                `  🔄  ${key}: "${value}" (PACE STRING - CONVERT TO SECONDS)`
              );
            }
          }
      ...
      updateLastResult({
        ...
        message: existingActivity
          ...
          : 'Activity not in database (as expected)',
      ...
      const safeNumber = (
        ...
        if (typeof value === 'string') {
          ...
          if (hasPaceIndicators) {
            console.log(
              `⚠️ Skipping pace string for ${fieldName}: "${value}" (will return null)`
            );
            return null;
          }
        ...
        if (isNaN(num)) {
          console.log(
            `⚠️ Cannot convert to number for ${fieldName}: "${value}" (type: ${typeof value})`
          );
          return null;
        }
        return num;
      ...
      const activityData = {
        ...
        total_elevation_gain: safeNumber(
          targetActivity.total_elevation_gain,
          'total_elevation_gain'
        ),
        average_speed: safeNumber(
          targetActivity.average_speed,
          'average_speed'
        ),
        max_speed: safeNumber(targetActivity.max_speed, 'max_speed'),
        average_heartrate:
          safeInteger(targetActivity.average_heartrate, 'average_heartrate') ||
          0,
        max_heartrate:
          safeInteger(targetActivity.max_heartrate, 'max_heartrate') || 0,
        has_heartrate: Boolean(targetActivity.has_heartrate),
        average_watts:
          safeInteger(targetActivity.average_watts, 'average_watts') || 0,
        max_watts: safeInteger(targetActivity.max_watts, 'max_watts') || 0,
        weighted_average_watts:
          safeInteger(
            targetActivity.weighted_average_watts,
            'weighted_average_watts'
          ) || 0,
        kilojoules: safeInteger(targetActivity.kilojoules, 'kilojoules') || 0,
        has_power: Boolean(
          targetActivity.average_watts || targetActivity.max_watts
        ), // Computed field
        trainer: Boolean(targetActivity.trainer),
        commute: Boolean(targetActivity.commute),
        manual: Boolean(targetActivity.manual),
        achievement_count:
          safeInteger(targetActivity.achievement_count, 'achievement_count') ||
          0,
        kudos_count:
          safeInteger(targetActivity.kudos_count, 'kudos_count') || 0,
        comment_count:
          safeInteger(targetActivity.comment_count, 'comment_count') || 0,
        ...
        week_number: (() => {
          ...
          const weekNumber = Math.ceil(
            (activityDate.getTime() - yearStart.getTime()) /
              (7 * 24 * 60 * 60 * 1000)
          );
          return safeInteger(weekNumber);
        })(),
        month_number: targetActivity.start_date
          ? safeInteger(new Date(targetActivity.start_date).getMonth() + 1)
          : null,
        year_number: targetActivity.start_date
          ? safeInteger(new Date(targetActivity.start_date).getFullYear())
          : null,
        day_of_week: targetActivity.start_date
          ? safeInteger(new Date(targetActivity.start_date).getDay())
          : null,

        // Calculate average pace in seconds per km (for numeric storage)
        average_pace:
          targetActivity.distance && targetActivity.moving_time
            ? safeInteger(
                targetActivity.moving_time / (targetActivity.distance / 1000)
              )
            : null,

        // Calculate elevation per km
        elevation_per_km:
          targetActivity.distance && targetActivity.total_elevation_gain
            ? safeNumber(
                (targetActivity.total_elevation_gain * 1000) /
                  targetActivity.distance
              )
            : null,
        ...
        efficiency_score:
          targetActivity.distance && targetActivity.moving_time
            ? safeNumber(
                targetActivity.distance / (targetActivity.moving_time / 60)
              )
            : null,
      };
        ...
        Object.entries(activityData).filter(([key, value]) => {
          ...
          if (typeof value === 'string') {
            ...
            if (pacePattern.test(value)) {
              console.error(
                `🚫 FILTERING OUT pace string in field '${key}': "${value}"`
              );
              return false;
            }
          }
      ...
      console.log(
        '🔍 Fields in raw targetActivity:',
        Object.keys(targetActivity)
      );
      console.log(
        '🔍 Fields in processed activityData:',
        Object.keys(activityData)
      );
      console.log(
        '🔍 Fields in safeActivityData:',
        Object.keys(safeActivityData)
      );

      // Check for any unmapped fields that might contain pace strings
      const unmappedFields = Object.keys(targetActivity).filter(
        key =>
          !Object.keys(activityData).includes(key) &&
          typeof targetActivity[key] === 'string' &&
          (targetActivity[key].includes('/km') ||
            targetActivity[key].includes('/mi') ||
            targetActivity[key].includes(':'))
      );

      if (unmappedFields.length > 0) {
        console.error(
          `🚨 UNMAPPED FIELDS WITH PACE STRINGS:`,
          unmappedFields.map(key => ({
            field: key,
      ...
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={debugSpecificActivity}
              size="sm"
              disabled={isDebugging || !accessToken || !user}
            >
      ...
      <CardContent>
        <div className="space-y-3">
          <p className="text-sm text-gray-600">
            This will trace exactly what happens when trying to store your
            &quot;Afternoon Run&quot; activity in the database.
          </p>
