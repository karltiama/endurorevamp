      ...
      <CardContent className="space-y-2 text-xs">
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <Badge variant={isLoadingWeeklyPlan ? 'destructive' : 'default'}>
              {isLoadingWeeklyPlan ? 'Yes' : 'No'}
            </Badge>
          </div>
          <div>
            <strong>Workouts Count:</strong>{' '}
            {
              Object.values(weeklyPlan?.workouts || {}).filter(w => w !== null)
                .length
            }
          </div>
        </div>
        ...
        <div className="border-t pt-2">
          ...
          <div className="mt-1 space-y-1">
            {weeklyPlan?.workouts &&
              Object.entries(weeklyPlan.workouts).map(([day, workout]) => (
                <div key={day} className="flex justify-between">
                  <span>Day {day}:</span>
                  <span>
                    {workout
                      ? `${(workout as { type?: string; sport?: string; duration?: number }).type} ${(workout as { type?: string; sport?: string; duration?: number }).sport} (${(workout as { type?: string; sport?: string; duration?: number }).duration}min)`
                      : 'Rest'}
                  </span>
                </div>
              ))}
