...
export function DatabaseActivityChecker({
  userId,
}: DatabaseActivityCheckerProps) {
  const {
    data: activities,
    isLoading,
    error,
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          <h4 className="font-medium mb-2">
            📊 Latest 5 Activities from Database:
          </h4>
          {latestActivities.length > 0 ? (
            ...
            <div className="text-gray-600 bg-yellow-50 p-3 rounded border border-yellow-200">
              ⚠️ No activities found in database. You need to sync your Strava
              data first.
            </div>
          )}
        </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-green-800">
              Data Source: Database (useUserActivities)
            </span>
          </div>
          <div className="text-sm text-green-600 mt-1">
            This component reads directly from your local database, NOT from
            Strava API.
          </div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded p-3">
          ...
          <div className="text-sm space-y-1">
            <div>
              1. **No activities?** → Use the Sync Dashboard above to sync from
              Strava
            </div>
            <div>
              2. **Missing latest run?** → Click &quot;Refresh&quot; or sync
              recent activities
            </div>
            <div>
              3. **Components showing different data?** → Click &quot;Clear
              Cache&quot; to reset everything
            </div>
          </div>
        </div>
      </CardContent>
