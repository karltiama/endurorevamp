...
export function WeekBoundaryDebugger({ userId }: WeekBoundaryDebuggerProps) {
    ...
    setDebugInfo({
      ...
      currentWeek: {
        ...
        activities: thisWeekActivities.map(a => ({
          ...
          training_stress_score: (a as { training_stress_score?: number })
  ...
  const currentWeekTSS = debugInfo.currentWeek.activities.reduce(
    (sum: number, a: ActivityDebugInfo) => sum + (a.training_stress_score || 0),
    0
  );
      ...
      <CardContent className="space-y-4">
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm space-y-1">
            ...
            <p>
              <strong>Total TSS:</strong> {currentWeekTSS}
            </p>
          </div>
            ...
            <div className="mt-3">
              ...
              <div className="space-y-1">
                {debugInfo.currentWeek.activities.map(
                  (activity: ActivityDebugInfo, index: number) => (
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <strong>Start:</strong>{' '}
              {new Date(debugInfo.nextWeek.start).toLocaleString()}
            </p>
            <p>
              <strong>End:</strong>{' '}
              {new Date(debugInfo.nextWeek.end).toLocaleString()}
            </p>
            <p>
              <strong>Activities:</strong>{' '}
              {debugInfo.nextWeek.activities.length}
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-4 bg-blue-50">
          ...
          <div className="text-sm space-y-1">
            <p>
              ...
              {debugInfo.dateCalculation.currentDay} (0=Sunday, 1=Monday, etc.)
            </p>
            <p>
              <strong>Current Date:</strong>{' '}
              {debugInfo.dateCalculation.currentDate}
            </p>
            <p>
              <strong>Week Start Calculation:</strong>{' '}
              {debugInfo.dateCalculation.weekStartCalculation}
            </p>
            <p>
              <strong>Explanation:</strong>{' '}
              {debugInfo.dateCalculation.explanation}
            </p>
          </div>
        </div>
            ...
            {debugInfo.allActivities.map(
              (activity: ActivityDebugInfo, index: number) => (
                <div key={index} className="text-xs bg-gray-50 p-2 rounded">
                  ...
                  <p>Sport: {activity.sport_type}</p>
                  <p>
                    TSS: {activity.training_stress_score || 'Not calculated'}
                  </p>
                </div>
              )
            )}
