...
export function SyncProcessDebugger() {
  ...
  const runFullDebug = async () => {
    ...
    try {
      ...
      if (updatedDbError) {
        ...
        const newCount =
          (updatedActivities?.length || 0) - (currentActivities?.length || 0);
        updateStep('post-sync-check', {
          status: 'success',
  ...
  const checkActivities = async () => {
    ...
    const { data: activities, error } = await supabase
      .from('activities')
      .select(
        'id, name, sport_type, start_date_local, strava_activity_id, distance, moving_time'
      )
      .order('start_date', { ascending: false })
      .limit(10);
  ...
  const debugMissingActivity = async () => {
    ...
    try {
      ...
      try {
        ...
        const { data: updatedDbActivities, error: updatedDbError } =
          await supabase
            .from('activities')
            .select(
              'id, name, sport_type, start_date_local, strava_activity_id'
            )
            .gte('start_date_local', '2024-07-22')
            .lte('start_date_local', '2024-07-22T23:59:59')
            .order('start_date_local', { ascending: false });
        ...
        if (updatedDbActivities && updatedDbActivities.length > 0) {
          ...
          updateStep('missing-activity-debug', {
            status: 'error',
            details:
              "❌ Still no 7/22 activities found. The activity may not exist in Strava or there's an API issue.",
      ...
      <div className="mb-6 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <span
              className={`ml-1 font-medium ${isSyncing ? 'text-blue-600' : 'text-gray-600'}`}
            >
      ...
      {activitiesData.length > 0 && (
        <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
          <h3 className="text-sm font-medium text-green-700 mb-2">
            📊 Database Activities ({activitiesData.length})
          </h3>
      ...
      <div className="space-y-3">
        ...
        {debugSteps.length === 0 ? (
          <p className="text-sm text-gray-500 italic">
            No debug steps yet. Click "Run Full Debug" to start.
          </p>
        ) : (
