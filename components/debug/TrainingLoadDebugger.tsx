...
export function TrainingLoadDebugger() {
  ...
  const { data: trainingLoadData } = useTrainingLoad(user?.id || '', { days: 90 })
  ...
  const analyzeDates = () => {
    ...
    try {
      ...
      activities.forEach(activity => {
        const date = (activity.start_date_local || activity.start_date || '').split('T')[0] // Get just the date part
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
                ...
                <div key={index} className="border rounded-lg p-4 bg-red-50">
                  ...
                  <div className="space-y-2">
                    {item.activities.map((activity, actIndex) => (
                      <div key={actIndex} className="text-sm bg-white p-2 rounded border">
                ...
                <div key={index} className="border rounded-lg p-4 bg-orange-50">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      Load: {trend.dailyLoad}, CTL: {trend.ctl}, ATL: {trend.atl}
                    </Badge>
                  </div>
                </div>
              ))}
          ...
          {dateAnalysis.length === 0 && trendsAnalysis.length === 0 && !isAnalyzing && (
            <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <h4 className="font-medium text-green-800 mb-2">✅ No Issues Found</h4>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div className="p-3 bg-gray-50 rounded">
                ...
                <div className="text-2xl font-bold">{trainingLoadData?.loadPoints?.length || 0}</div>
              </div>
              <div className="p-3 bg-gray-50 rounded">
              ...
              <div className="p-3 bg-gray-50 rounded">
                ...
                <div className="text-2xl font-bold">
                  {new Set(activities.map(a => a.start_date_local.split('T')[0])).size}
                </div>
              </div>
            </div>
          </div>
