...
export function WeeklyTrainingLoadDebugger({
  userId,
}: WeeklyTrainingLoadDebuggerProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useUserActivities(currentUserId || '');
  const { data: personalizedTSSTarget } = usePersonalizedTSSTarget(
    currentUserId || ''
  );
  ...
  useEffect(() => {
    ...
    const currentWeekTSS = thisWeekActivities.reduce((sum, activity) => {
      const tss =
        (activity as ActivityWithTrainingData).training_stress_score || 0;
      return sum + tss;
    }, 0);
    ...
    const activitiesWithTSS = thisWeekActivities.filter(
      activity =>
    ...
    const progressPercentage =
      targetTSS > 0 ? Math.round((currentWeekTSS / targetTSS) * 100) : 0;
    ...
    setDebugInfo({
      ...
      currentWeek: {
        ...
        activities: thisWeekActivities.map(a => ({
          ...
          training_stress_score: (a as ActivityWithTrainingData)
      ...
      <CardContent className="space-y-4">
        <div className="text-sm text-gray-600">
          <p>
            ...
            {new Date(debugInfo.now).toLocaleString()}
          </p>
        </div>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <strong>Start:</strong>{' '}
              {new Date(debugInfo.currentWeek.start).toLocaleString()}
            </p>
            <p>
              <strong>End:</strong>{' '}
              {new Date(debugInfo.currentWeek.end).toLocaleString()}
            </p>
            <p>
              <strong>Activities:</strong>{' '}
              {debugInfo.currentWeek.activities.length}
            </p>
            <p>
              <strong>Total TSS:</strong> {debugInfo.currentWeek.totalTSS}
            </p>
            <p>
              <strong>Target TSS:</strong> {debugInfo.targetTSS}
            </p>
            <p>
              <strong>Progress:</strong>{' '}
              {debugInfo.currentWeek.progressPercentage}%
            </p>
            <p>
              <strong>Activities with TSS:</strong>{' '}
              {debugInfo.currentWeek.activitiesWithTSS}
            </p>
          </div>
          ...
          {needsTSSUpdate && (
            <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded">
              <p className="text-xs text-yellow-800 flex items-center gap-1">
                ...
                ⚠️ No activities have TSS calculated. This will affect progress
                accuracy.
              </p>
            </div>
          )}
          ...
          {debugInfo.currentWeek.activities.length > 0 ? (
            <div className="mt-3">
              ...
              <div className="space-y-1">
                {debugInfo.currentWeek.activities.map(
                  (activity: any, index: number) => (
                    <div key={index} className="text-xs bg-gray-50 p-2 rounded">
                      ...
                      <p>
                        TSS:{' '}
                        {activity.training_stress_score || 'Not calculated'}
                      </p>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <strong>Start:</strong>{' '}
              {new Date(debugInfo.previousWeek.start).toLocaleString()}
            </p>
            <p>
              <strong>End:</strong>{' '}
              {new Date(debugInfo.previousWeek.end).toLocaleString()}
            </p>
            <p>
              <strong>Activities:</strong>{' '}
              {debugInfo.previousWeek.activities.length}
            </p>
            <p>
              <strong>Total TSS:</strong> {debugInfo.previousWeek.totalTSS}
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="text-sm space-y-1">
            <p>
              <strong>Current Week TSS:</strong>{' '}
              {debugInfo.currentWeek.totalTSS}
            </p>
            <p>
              <strong>Previous Week TSS:</strong>{' '}
              {debugInfo.previousWeek.totalTSS}
            </p>
            <p>
              <strong>Target TSS:</strong> {debugInfo.targetTSS}
            </p>
            ...
            <div className="mt-2">
              <Badge
                variant={
                  debugInfo.currentWeek.progressPercentage >= 100
                    ? 'default'
                    : 'secondary'
                }
              >
