    ...
    try {
      ...
      activitiesData?.forEach(activity => {
        ...
        if (activity.distance <= 0)
          results.dataQualityIssues.push(`${activity.name}: Invalid distance`);
        if (activity.moving_time <= 0)
          results.dataQualityIssues.push(`${activity.name}: Invalid time`);
        if (
          activity.average_heartrate &&
          (activity.average_heartrate < 30 || activity.average_heartrate > 220)
        ) {
          results.dataQualityIssues.push(
            `${activity.name}: Invalid heart rate`
          );
        }
      });
    ...
    <Card className="w-full">
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          Tests that essential activity data is parsing, storing, and displaying
          correctly
        </p>
      </CardHeader>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-yellow-50 p-3 rounded">
                ...
                <div className="text-lg font-bold text-yellow-900">
                  {validation.performanceFieldsPresent}/
                  {validation.totalActivities}
                </div>
              </div>
              <div className="bg-red-50 p-3 rounded">
                ...
                <div className="text-lg font-bold text-red-900">
                  {validation.dataQualityIssues.length}
                </div>
              </div>
            </div>
