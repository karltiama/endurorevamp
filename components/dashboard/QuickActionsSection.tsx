...
const calculateTrainingState = (
  ...
  const averageRPE =
    recentActivities.reduce((sum, activity) => {
      const rpe =
        (activity as ActivityWithTrainingData).perceived_exertion || 5;
      return sum + rpe;
  ...
  const daysSinceLastWorkout =
    activities && activities.length > 0
      ? Math.floor(
          (Date.now() - new Date(activities[0].start_date).getTime()) /
            (24 * 60 * 60 * 1000)
        )
      : 7;
...
const generateContextualActions = (
  ...
  if (
    recentActivity &&
    !(recentActivity as ActivityWithTrainingData).perceived_exertion
  ) {
    actions.push({
      id: 'log-rpe',
      title: 'Log RPE',
...
export function QuickActionsSection({ userId }: QuickActionsSectionProps) {
  ...
  const contextualActions = useMemo((): QuickAction[] => {
    ...
    const recentActivity = activities.find(
      activity => new Date(activity.start_date) >= yesterday
    );

    const recentActivities = activities.filter(
      activity => new Date(activity.start_date) >= lastWeek
    );
    ...
    <>
      ...
      <RPELoggingModal
        ...
        activity={activities?.find(
          a =>
            new Date(a.start_date) >= new Date(Date.now() - 24 * 60 * 60 * 1000)
        )}
      />
    </>
  );
