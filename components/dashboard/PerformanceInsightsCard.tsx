...
const calculatePaceImprovement = (
  recent: StravaActivity[],
...
const calculateTrainingLoadTrend = (
  recent: StravaActivity[],
  previous: StravaActivity[]
) => {
  const getAverageLoad = (activities: StravaActivity[]) => {
    ...
    const totalLoad = activities.reduce(
      (sum: number, activity: StravaActivity) => {
        const load =
          (activity as ActivityWithTrainingData).training_load_score ||
          estimateTrainingLoad(activity);
        return sum + load;
      },
      0
    );
...
const findRecentAchievements = (
  activities: StravaActivity[],
  preferences: { distance: 'km' | 'miles' }
): Achievement[] => {
  ...
  if (longestRecent.distance > 0) {
    ...
    const previousLongest = olderActivities.reduce(
      (max: StravaActivity, activity: StravaActivity) =>
        activity.distance > max.distance ? activity : max,
      { distance: 0 } as StravaActivity
    );
...
const calculateWeeklyDistance = (
  ...
  const getWeeklyDistance = (activities: StravaActivity[]) =>
    activities.reduce(
      (sum: number, activity: StravaActivity) => sum + activity.distance,
      0
    ) / 1000; // Convert to km
...
const calculateAverageIntensity = (
  ...
  const getAverageIntensity = (activities: StravaActivity[]) => {
    if (activities.length === 0) return 0;
    const totalIntensity = activities.reduce(
      (sum: number, activity: StravaActivity) => {
  ...
  const change =
    previousIntensity > 0
      ? ((currentIntensity - previousIntensity) / previousIntensity) * 100
      : 0;
  const trend: 'up' | 'down' | 'stable' =
...
const calculateRecentPerformance = (
  ...
  const improvement =
    previous7Days > 0 ? ((last7Days - previous7Days) / previous7Days) * 100 : 0;
...
export function PerformanceInsightsCard({
  ...
  const performanceInsights = useMemo((): PerformanceInsights | null => {
    ...
    const paceImprovement = calculatePaceImprovement(
      recentActivities,
      previousActivities
    );

    // Calculate training load trend
    const trainingLoadTrend = calculateTrainingLoadTrend(
      recentActivities,
      previousActivities
    );
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-center">
            ...
            <div className="text-xl font-bold text-blue-600">
              {performanceInsights.weeklyDistance.current > 0
                ? parseFloat(
                    formatDistance(
                      performanceInsights.weeklyDistance.current * 1000,
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
              <Zap
                className={`h-4 w-4 ${
                  performanceInsights.trainingLoadTrend.trend === 'increasing'
                    ? 'text-orange-600'
                    : performanceInsights.trainingLoadTrend.trend ===
                        'decreasing'
                      ? 'text-purple-600'
                      : 'text-gray-600'
                }`}
