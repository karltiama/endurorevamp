...
export function LastActivityDeepDive({ userId }: LastActivityDeepDiveProps) {
  ...
  const activityAnalysis = useMemo(() => {
    ...
    const avgDistance =
      sameTypeActivities.reduce((sum, a) => sum + a.distance, 0) /
      sameTypeActivities.length;

    // Performance analysis
    const currentPace =
      lastActivity.moving_time / (lastActivity.distance / 1000); // seconds per km
    const paceImprovement = previousActivity
      ? previousActivity.moving_time / (previousActivity.distance / 1000) -
        currentPace
      : 0;
    const distanceImprovement = previousActivity
      ? lastActivity.distance - previousActivity.distance
      : 0;
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Loading your latest activity analysis...
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div>
            <CardTitle className="flex items-center gap-2">
              <span className="text-2xl">
                {getActivityIcon(lastActivity.sport_type, lastActivity.trainer)}
              </span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {formatDistanceWithUnits(lastActivity.distance)}
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {formatDuration(lastActivity.moving_time)}
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {activityAnalysis
                    ? formatPaceWithUnits(activityAnalysis.currentPace)
                    : 'N/A'}
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {Math.round(lastActivity.total_elevation_gain || 0)}m
                </div>
              </div>
            </div>
              ...
              <>
                ...
                <div className="space-y-4">
                  <h4 className="font-semibold">
                    Performance vs Previous {lastActivity.sport_type}
                  </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <TrendingUp
                          className={`h-4 w-4 ${activityAnalysis.distanceImprovement >= 0 ? 'text-green-500' : 'text-red-500'}`}
                        />
                      </div>
                      <div
                        className={`text-lg font-bold ${activityAnalysis.distanceImprovement >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {activityAnalysis.distanceImprovement >= 0 ? '+' : ''}
                        {formatDistanceWithUnits(
                          activityAnalysis.distanceImprovement
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <TrendingUp
                          className={`h-4 w-4 ${activityAnalysis.paceImprovement >= 0 ? 'text-green-500' : 'text-red-500'}`}
                        />
                      </div>
                      <div
                        className={`text-lg font-bold ${activityAnalysis.paceImprovement >= 0 ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {activityAnalysis.paceImprovement >= 0
                          ? 'Faster'
                          : 'Slower'}{' '}
                        by{' '}
                        {Math.abs(activityAnalysis.paceImprovement).toFixed(0)}
                        s/{preferences.distance === 'miles' ? 'mi' : 'km'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={activityAnalysis.effortLevel}
                      className="h-2"
                    />
              ...
              <>
                  ...
                  <div className="grid gap-4">
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                      ...
                      <p className="text-sm text-blue-700">
                        This was your{' '}
                        {activityAnalysis.totalActivitiesOfType === 1
                          ? 'first'
                          : `${activityAnalysis.totalActivitiesOfType}${getOrdinalSuffix(activityAnalysis.totalActivitiesOfType)}`}{' '}
                        {lastActivity.sport_type.toLowerCase()} activity.
                        {activityAnalysis.distanceImprovement > 1000 &&
                          ' Great distance improvement!'}
                        {activityAnalysis.paceImprovement > 10 &&
                          ' Nice pace improvement!'}
                        {activityAnalysis.isPersonalBest &&
                          ' This is a new personal best for distance!'}
                      </p>
                    </div>
                    ...
                    <div className="bg-green-50 p-4 rounded-lg border border-green-100">
                      ...
                      <ul className="text-sm text-green-700 space-y-1">
                        {activityAnalysis.effortLevel > 80 && (
                          <li>
                            • Consider a recovery day after this high-intensity
                            session
                          </li>
                        )}
                        ...
                        {activityAnalysis.distanceImprovement > 2000 && (
                          <li>
                            • Great distance increase! Monitor how you feel for
                            recovery
                          </li>
                        )}
                        {lastActivity.total_elevation_gain &&
                          lastActivity.total_elevation_gain > 500 && (
                            <li>
                              • Solid elevation gain - great for building
                              strength
                            </li>
                          )}
                ...
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-100">
                  ...
                  <p className="text-sm text-orange-700">
                    ...
                    {activityAnalysis.effortLevel > 80
                      ? '24-48 hours'
                      : activityAnalysis.effortLevel > 60
                        ? '12-24 hours'
                        : '6-12 hours'}{' '}
