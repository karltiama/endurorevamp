...
import { Activity, BarChart3, RefreshCw, AlertTriangle } from 'lucide-react';
...
export function WeeklyTrainingLoadWidget({
  userId,
}: WeeklyTrainingLoadWidgetProps) {
  ...
  useEffect(() => {
    ...
    queryClient.invalidateQueries({
      queryKey: ['personalized-tss-target', userId],
    });
  }, [userId, queryClient]);
  ...
  const weeklyTrainingLoad = useMemo((): WeeklyTrainingLoad | null => {
    ...
    const currentTSS = thisWeekActivities.reduce((sum, activity) => {
      const tss =
        ...
        estimateTSS(activity as ActivityWithTrainingData);
      return sum + tss;
    }, 0);
    ...
    const weeklyTrend =
      currentTSS > previousTSS * 1.1
        ? 'up'
        : currentTSS < previousTSS * 0.9
          ? 'down'
          : 'stable';
  ...
  const needsTSSUpdate =
    activities?.some(activity => {
      ...
      return (
        activityWithTraining.training_stress_score === null ||
        activityWithTraining.training_stress_score === undefined
      );
    }) || false;
    ...
    <Card className="h-full flex flex-col">
      ...
      <CardContent className="space-y-4 flex-1 h-full flex flex-col justify-between">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="text-sm text-gray-600">Weekly TSS Progress</div>
          <Progress
