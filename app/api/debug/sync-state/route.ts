  ...
  try {
    ...
    switch (action) {
      case 'add_sync':
        ...
        console.log(
          `➕ Adding sync: ${currentSyncs} -> ${updateData.sync_requests_today}`
        );
        break;
      ...
      case 'remove_sync':
        ...
        console.log(
          `➖ Removing sync: ${currentSyncsRemove} -> ${updateData.sync_requests_today}`
        );
        break;
      ...
      case 'reset_timer':
        ...
        updateData.last_activity_sync = new Date(
      ...
      case 'add_error':
        ...
        console.log(
          `❌ Adding error: ${currentErrors} -> ${updateData.consecutive_errors}`
        );
        break;
    ...
    const { error: updateError } = await supabase.from('sync_state').upsert({
      user_id: userId,
      ...updateData,
