...
export async function GET() {
  try {
    ...
    const now = Date.now()
    const tokenExpiry = tokens?.expires_at ? new Date(tokens.expires_at).getTime() : null
    ...
    return NextResponse.json({
      ...
      stravaConnection: {
        ...
        athleteName: tokens ? `${tokens.athlete_firstname} ${tokens.athlete_lastname}` : null,
        tokenExpiresIn: tokenExpiresIn ? Math.floor(tokenExpiresIn / 1000) : null,
