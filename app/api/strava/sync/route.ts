...
async function checkCanSync(
  ...
  if (syncState.last_activity_sync) {
    ...
    if (lastSync > oneHourAgo) {
      ...
      if (!hadNewActivities) {
        console.log(
          '❌ Too soon since last sync with no new activities (1 hour cooldown)'
        );
        const minutesLeft = Math.ceil(
          (lastSync.getTime() - oneHourAgo.getTime()) / (1000 * 60)
        );
        return {
          canSync: false,
        ...
        console.log(
          '✅ Last sync had new activities, allowing immediate re-sync'
        );
      }
    }
  }
