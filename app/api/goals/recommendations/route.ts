...
export async function GET(request: NextRequest) {
  try {
    ...
    if (activitiesError) {
      console.error('Error fetching recent activities for recommendations:', activitiesError)
    ...
    if (goalsError) {
      console.error('Error fetching current goals for recommendations:', goalsError)
    ...
    if (typesError) {
      console.error('Error fetching goal types for recommendations:', typesError)
      return NextResponse.json({ error: 'Failed to fetch goal types' }, { status: 500 })
    }
...
function generateGoalRecommendations(
  ...
  const hasDistanceGoal = currentGoals.some(g => g.goal_type?.metric_type === 'total_distance')
  if (!hasDistanceGoal && totalDistance > 0) {
    const distanceGoalType = goalTypes.find(gt => gt.metric_type === 'total_distance')
    if (distanceGoalType) {
  ...
  const hasPaceGoal = currentGoals.some(g => g.goal_type?.metric_type === 'average_pace')
  if (!hasPaceGoal && averagePace > 0) {
    const paceGoalType = goalTypes.find(gt => gt.metric_type === 'average_pace')
    if (paceGoalType) {
  ...
  const hasFrequencyGoal = currentGoals.some(g => g.goal_type?.metric_type === 'run_count')
  ...
  if (recommendations.length === 0) {
    const generalGoalType = goalTypes.find(gt => gt.metric_type === 'total_distance')
    if (generalGoalType) {
      recommendations.push({
        type: 'general',
        goalType: generalGoalType,
        suggestedTarget: 50, // 50km total distance
        reasoning: 'Start with a general fitness goal to track your progress and build momentum.',
        priority: 'low'
      })
    }
  }
