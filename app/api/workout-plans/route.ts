...
export async function GET() {
  try {
    ...
    const { data: planData, error } = await supabase.rpc('get_current_week_plan', {
      user_uuid: user.id
    })
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('POST /api/workout-plans: Received weeklyPlan:', {
      ...
      workoutsCount: Object.values(weeklyPlan?.workouts || {}).filter(w => w !== null).length,
    ...
    if (!weeklyPlan) {
      ...
      return NextResponse.json({ error: 'Weekly plan is required' }, { status: 400 })
    }
    ...
    console.log('POST /api/workout-plans: plan_data JSON length:', planDataJson.length)
    console.log('POST /api/workout-plans: plan_data JSON preview:', planDataJson.substring(0, 500) + '...')
    ...
    if (error) {
      console.error('POST /api/workout-plans: Error saving workout plan:', error)
    ...
    console.log('POST /api/workout-plans: Successfully saved plan with ID:', planId)
...
export async function DELETE() {
  try {
    ...
    console.log('DELETE /api/workout-plans: Existing plans for this week:', existingPlans)
