...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log(`🔔 [${webhookId}] Webhook received from ${request.headers.get('user-agent')}`)
    ...
    if (!event.object_type || !event.owner_id) {
      console.error(`❌ [${webhookId}] Invalid webhook payload - missing required fields`)
...
async function handleActivityEvent(event: { aspect_type: string; owner_id: number; object_id: number }, webhookId: string = 'unknown') {
  ...
  try {
    ...
    if (!tokens) {
      console.log(`⚠️ [${webhookId}] No user found for Strava athlete ID: ${owner_id}`)
      return
    }

    switch (aspect_type) {
      case 'create':
        console.log(`🏃 [${webhookId}] New activity ${object_id} for user ${tokens.user_id}`)
      ...
      case 'update':
        console.log(`📝 [${webhookId}] Activity ${object_id} updated for user ${tokens.user_id}`)
      ...
      case 'delete':
        console.log(`🗑️ [${webhookId}] Activity ${object_id} deleted for user ${tokens.user_id}`)
        ...
        if (deleteError) {
          console.error(`❌ [${webhookId}] Failed to delete activity:`, deleteError)
        } else {
          console.log(`✅ [${webhookId}] Activity ${object_id} deleted from database`)
        }
        break
    }
  } catch (error) {
...
async function handleAthleteEvent(event: { aspect_type: string; owner_id: number; updates?: { authorized?: string } }, webhookId: string = 'unknown') {
  ...
  if (aspect_type === 'update' && event.updates?.authorized === 'false') {
    ...
    if (error) {
      ...
      console.log(`✅ [${webhookId}] Tokens removed for deauthorized user ${owner_id}`)
    }
  }
}
