...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { maxHeartRate, zoneModel = '5-zone', sportFilter } = body;
    ...
    if (
      ...
      analysis.suggestedZoneModel =
        customModels.find(m =>
          m.name.toLowerCase().includes(zoneModel.toLowerCase())
        ) || customModels[0];

      analysis.alternativeModels = customModels.filter(
        m => m.name !== analysis.suggestedZoneModel.name
      );

      analysis.recommendations.unshift(
        `Using custom max heart rate of ${maxHeartRate} BPM`
      );
    }
