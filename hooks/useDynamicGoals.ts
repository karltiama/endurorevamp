...
export const useDynamicGoals = (userId: string, options: UseDynamicGoalsOptions = {}): UseDynamicGoalsReturn => {
  ...
  const dynamicGoalsQuery = useQuery({
    ...
    queryFn: async (): Promise<{ suggestions: DynamicGoalSuggestion[]; userProfile: UserPerformanceProfile }> => {
    ...
    enabled: enabled && !!userId && !isLoadingTypes && !isLoadingGoals && goalTypes.length > 0,
    staleTime,
    retry: 1,
  ...
  return {
    ...
    error: dynamicGoalsQuery.error ? (dynamicGoalsQuery.error as Error).message : null,
 ...
 */
export const usePrioritizedDynamicGoals = (userId: string, options: UseDynamicGoalsOptions = {}) => {
 ...
 */
export const useNewCategoryDynamicGoals = (userId: string, options: UseDynamicGoalsOptions = {}) => {
  const { data: goalsData } = useUserGoals();
  
  // Get categories user already has active goals in
  const activeCategories = goalsData?.goals
