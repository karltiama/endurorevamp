  ...
  return useMutation<ZoneAnalysisResult, ZoneAnalysisError, CustomZoneParams>({
    mutationFn: async (params: CustomZoneParams) => {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || 'Failed to perform custom zone analysis'
        );
      }
...
export function useZoneCalculations() {
  ...
  const calculateZoneFromHeartRate = (heartRate: number) => {
    ...
    const zone = analysis.suggestedZoneModel.zones.find(
      z => heartRate >= z.minHR && heartRate <= z.maxHR
    );
  ...
  const getZoneDistribution = (
    heartRateData: { hr: number; duration: number }[]
  ) => {
    ...
    const totalTime = heartRateData.reduce(
  ...
  const getTargetZoneRecommendation = (workoutType: string) => {
    ...
    const targetZone =
      recommendations[
        workoutType.toLowerCase() as keyof typeof recommendations
      ];
    return (
      analysis.suggestedZoneModel.zones.find(z => z.number === targetZone) ||
      null
    );
  };
