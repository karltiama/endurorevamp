...
export function useProfileAnalysis(userId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<ProfileAnalysis | null> => {
      const profile = await TrainingProfileService.getCompleteProfile(
        userId,
        false
      );
...
export function useCalculateThresholds() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { userId }) => {
      ...
      queryClient.invalidateQueries({
        queryKey: TRAINING_PROFILE_KEYS.thresholds(userId),
      });
      queryClient.invalidateQueries({
        queryKey: TRAINING_PROFILE_KEYS.profile(userId),
      });
...
export function usePersonalizedTSSTarget(userId: string) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return TrainingProfileService.calculatePersonalizedTSSTarget(
        profile.profile,
        profile.preferences
      );
    },
