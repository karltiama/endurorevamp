...
export function useEnhancedWorkoutPlanning({ userId, includeWeather = false }: UseEnhancedWorkoutPlanningOptions) {
  // Get user activities
  ...
  const { data: trainingLoadData, isLoading: trainingLoadLoading, error: trainingLoadError } = useTrainingLoad(userId, { days: 21 })
  
  // Get unit preferences
  const { preferences: unitPreferences } = useUnitPreferences()

  // Get user goals
  const { data: goalsData, isLoading: goalsLoading, error: goalsError } = useUserGoals()
  const goals = goalsData?.goals || []

  // Get saved workout plan from database
  const { data: savedPlanData, isLoading: savedPlanLoading, error: savedPlanError } = useQuery({
  ...
  const { data: weeklyPlan, isLoading: weeklyPlanLoading } = useQuery({
    queryKey: ['enhanced-workout-planning', 'weekly-plan', userId, planningActivities.length, trainingLoadData?.metrics, goals.length, unitPreferences, savedPlanData?.weeklyPlan?.id || 'no-saved-plan'],
    queryFn: async () => {
      console.log('useEnhancedWorkoutPlanning: Weekly plan queryFn called')
      console.log('useEnhancedWorkoutPlanning: savedPlanData?.weeklyPlan:', savedPlanData?.weeklyPlan?.id)
  ...
  return {
    ...
    isLoadingTodaysWorkout: todaysWorkoutLoading || activitiesLoading || trainingLoadLoading || goalsLoading,
    
    // Weekly plan
    weeklyPlan: weeklyPlan?.weeklyPlan || null,
    isLoadingWeeklyPlan: weeklyPlanLoading || activitiesLoading || trainingLoadLoading || goalsLoading || savedPlanLoading,
    
    // Loading states
    isLoading: activitiesLoading || trainingLoadLoading || goalsLoading || savedPlanLoading,
 ...
 */
function deriveTodaysWorkoutFromPlan(weeklyPlan: WeeklyWorkoutPlan): EnhancedWorkoutRecommendation | null {
...
async function generateNewWorkoutPlan(userId: string, queryClient: any): Promise<WeeklyWorkoutPlan | null> {
  try {
    ...
    const activitiesData = queryClient.getQueryData(['user', 'activities', userId])
    const goalsData = queryClient.getQueryData(['goals', 'user'])
    const trainingLoadData = queryClient.getQueryData(['training-load', userId, 21, activitiesData?.length || 0])
    
    console.log('generateNewWorkoutPlan: Activities from cache:', activitiesData?.length || 0)
    console.log('generateNewWorkoutPlan: Goals from cache:', goalsData?.goals?.length || 0)
    console.log('generateNewWorkoutPlan: Training load from cache:', trainingLoadData?.metrics)
    ...
    if (!activitiesData || !goalsData || !trainingLoadData?.metrics) {
      ...
      console.log('generateNewWorkoutPlan: activitiesData exists:', !!activitiesData)
      console.log('generateNewWorkoutPlan: goalsData exists:', !!goalsData)
      console.log('generateNewWorkoutPlan: trainingLoadData exists:', !!trainingLoadData)
      return null
    }
    ...
    console.log('generateNewWorkoutPlan: Planning activities (filtered):', planningActivities.length)
    ...
    if (!newPlan || !newPlan.workouts || Object.keys(newPlan.workouts).length === 0) {
      console.error('generateNewWorkoutPlan: Generated plan is invalid or empty')
      return null
    }
...
export function useWorkoutPlanManager(userId: string) {
  ...
  const updateWeeklyPlan = async (updatedPlan: WeeklyWorkoutPlan) => {
    try {
      ...
      console.log('updateWeeklyPlan: Plan data:', {
        ...
        workoutsCount: Object.values(updatedPlan.workouts).filter(w => w !== null).length,
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to save workout plan: ${response.status} - ${errorText}`)
      }
  ...
  const saveWorkoutPlan = async (plan: WeeklyWorkoutPlan) => {
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['enhanced-workout-planning', 'todays-workout'] })
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to reset workout plan: ${response.status} - ${errorText}`)
      }
      ...
      await queryClient.invalidateQueries({ queryKey: ['workout-plans', userId] })
      console.log('resetToRecommended: Invalidating enhanced-workout-planning query')
      await queryClient.invalidateQueries({ queryKey: ['enhanced-workout-planning'] })
      ...
      console.log('resetToRecommended: Forcing refetch of all enhanced-workout-planning queries')
      await queryClient.refetchQueries({ queryKey: ['enhanced-workout-planning'] })
      ...
      if (newPlan && newPlan.workouts) {
        ...
        if (saveResult.success) {
          console.log('resetToRecommended: New intelligent plan saved successfully')
          ...
          await queryClient.removeQueries({ queryKey: ['enhanced-workout-planning'] })
          
          // Force fresh refetch of all workout queries
          console.log('resetToRecommended: Forcing fresh refetch of all workout queries')
          ...
          console.log('resetToRecommended: Failed to save new intelligent plan')
          return { success: true, result, newPlan: null }
        }
      } else {
        console.log('resetToRecommended: No intelligent plan generated, will use fallback')
...
export function useWorkoutPlanAnalytics(weeklyPlan: WeeklyWorkoutPlan | null) {
  ...
  const workouts = Object.values(weeklyPlan.workouts).filter(w => w !== null) as EnhancedWorkoutRecommendation[]
  ...
  const sportDistribution = workouts.reduce((acc, workout) => {
