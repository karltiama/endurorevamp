...
export class TrainingLoadCalculator {
  ...
  calculateTRIMP(activity: Activity): number {
    ...
    const sportMultiplier =
      SPORT_MULTIPLIERS[
        activity.sport_type as keyof typeof SPORT_MULTIPLIERS
      ] || SPORT_MULTIPLIERS.Default;
  ...
  calculateWeightTrainingLoad(activity: ActivityWithTrainingData): number {
    ...
    const typeMultiplier =
      WEIGHT_TRAINING_MULTIPLIERS[exerciseType] ||
      WEIGHT_TRAINING_MULTIPLIERS.default;
   ...
   */
  private determineWeightTrainingType(
    activity: ActivityWithTrainingData
  ): keyof typeof WEIGHT_TRAINING_MULTIPLIERS {
  ...
  calculateLoadMetrics(loadPoints: TrainingLoadPoint[]): TrainingLoadMetrics {
    ...
    // Sort by date
    const sortedPoints = loadPoints.sort(
  ...
  processActivities(activities: Activity[]): TrainingLoadPoint[] {
    ...
    const loadPoints = Array.from(activitiesByDate.entries()).map(
      ([date, dayActivities]) => {
        // Aggregate all metrics for the day
        const totalTrimp = dayActivities.reduce(
          (sum, activity) => sum + this.calculateTRIMP(activity),
          0
        );
        const totalTss = dayActivities.reduce(
          (sum, activity) => sum + this.calculateTSS(activity),
          0
        );
        const totalLoad = dayActivities.reduce(
          (sum, activity) => sum + this.calculateNormalizedLoad(activity),
          0
        );
        ...
        return {
          ...
          activity: {
            ...
            sport_type:
              dayActivities.length > 1 ? 'Mixed' : primaryActivity.sport_type,
            ...
            avgHR: dayActivities.some(act => act.average_heartrate)
              ? Math.round(
                  dayActivities.reduce(
                    (sum, act) => sum + (act.average_heartrate || 0),
                    0
            ...
            avgPower: dayActivities.some(act => act.average_watts)
              ? Math.round(
                  dayActivities.reduce(
                    (sum, act) => sum + (act.average_watts || 0),
                    0
                  ) / dayActivities.length
                )
              : undefined,
  ...
  private calculateVariabilityIndex(activity: Activity): number {
    ...
    return (
      sportVI[activity.sport_type as keyof typeof sportVI] || sportVI.Default
    );
  }
  ...
  private getIntensityMultiplier(activity: Activity): number {
    ...
    if (activity.average_heartrate && this.athleteThresholds.maxHeartRate) {
      const hrRatio =
        activity.average_heartrate / this.athleteThresholds.maxHeartRate;
  ...
  private calculateRampRate(loadPoints: TrainingLoadPoint[]): number {
    ...
    const firstWeekAvg =
      firstWeek.reduce((sum, p) => sum + p.normalizedLoad, 0) / 7;
    const secondWeekAvg =
      secondWeek.reduce((sum, p) => sum + p.normalizedLoad, 0) / 7;

    return secondWeekAvg - firstWeekAvg;
  }

  private determineTrainingStatus(
    ctl: number,
    atl: number,
    tsb: number,
    rampRate: number
 ...
 */
export function estimateAthleteThresholds(
  activities: Activity[]
): AthleteThresholds {
  ...
  const maxHeartRate =
    maxHRs.length > 0 ? maxHRs[Math.floor(maxHRs.length * 0.05)] || 190 : 190;
  ...
  const restingHeartRate =
    avgHRs.length > 0 ? avgHRs[Math.floor(avgHRs.length * 0.05)] || 60 : 60;
  ...
  const functionalThresholdPower =
 ...
 */
export async function ensureActivitiesHaveTSS(
  userId: string
  ...
  try {
    ...
    const trainingLoadCalculator = new TrainingLoadCalculator(
      defaultThresholds
    );
 ...
 */
export async function getActivitiesWithTSS(
  userId: string,
  startDate?: Date,
  endDate?: Date
): Promise<Activity[]> {
 ...
 */
export async function calculateWeeklyTSS(
  userId: string,
  weekStart: Date
): Promise<{
  totalTSS: number;
  ...
  activities.forEach(activity => {
    ...
    const tss =
      (activity as Activity & { training_stress_score?: number })
  ...
  return {
    ...
    averageTSS:
      activities.length > 0 ? Math.round(totalTSS / activities.length) : 0,
    dailyBreakdown,
  };
}
