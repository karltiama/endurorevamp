...
export class TrainingProfileService {
  ...
  static async getCompleteProfile(
  ...
  ): Promise<CompleteTrainingProfile | null> {
    ...
    try {
      ...
      return {
        ...
        preferences:
          (preferences as UserTrainingPreferences) ||
          (await this.createDefaultPreferences(userId, useServerClient)),
  ...
  static async calculateThresholds(
    ...
    try {
      ...
      await supabase.from('threshold_calculation_history').insert({
        user_id: userId,
  ...
  static async autoUpdateFromCalculation(
    ...
    if (
      estimation.confidence_scores.max_heart_rate &&
      estimation.confidence_scores.max_heart_rate > 0.7
    ) {
      if (
        !profile?.profile.max_heart_rate ||
        profile.profile.max_hr_source === 'estimated'
      ) {
    ...
    if (
      estimation.confidence_scores.resting_heart_rate &&
      estimation.confidence_scores.resting_heart_rate > 0.6
    ) {
      if (
        !profile?.profile.resting_heart_rate ||
        profile.profile.resting_hr_source === 'estimated'
      ) {
        updateData.resting_heart_rate =
          estimation.estimated_values.resting_heart_rate;
      }
    }

    if (
      estimation.confidence_scores.functional_threshold_power &&
      estimation.confidence_scores.functional_threshold_power > 0.7
    ) {
      if (
        !profile?.profile.functional_threshold_power ||
        profile.profile.ftp_source === 'estimated'
      ) {
        updateData.functional_threshold_power =
          estimation.estimated_values.functional_threshold_power;
      }
    }

    // Calculate personalized TSS target
    if (profile?.profile) {
      const personalizedTSS = this.calculatePersonalizedTSSTarget(
        profile.profile,
        estimation
      );
      if (
        personalizedTSS !== profile.profile.weekly_tss_target &&
        profile.profile.tss_target_source === 'estimated'
      ) {
  ...
  static analyzeProfile(profile: CompleteTrainingProfile): ProfileAnalysis {
    ...
    if (tp.age) completeness += 5;
    else missing.push('age');
    if (tp.weight) completeness += 5;
    else missing.push('weight');
    if (tp.sex) completeness += 5;
    else missing.push('biological sex');
    ...
    if (tp.max_heart_rate) completeness += 15;
    else missing.push('max heart rate');
    if (tp.resting_heart_rate) completeness += 10;
    ...
    if (
      prefs.easy_percentage &&
      prefs.moderate_percentage &&
      prefs.hard_percentage
    )
      completeness += 5;
    if (prefs.recovery_priority) completeness += 5;

    // Determine confidence level
    const hasRecentCalculation =
      calculation_history.length > 0 &&
      calculation_history[0].confidence_score &&
      calculation_history[0].confidence_score > 0.7;

    const confidence_level =
      completeness >= 80 && hasRecentCalculation
        ? 'high'
        : completeness >= 60
          ? 'medium'
          : 'low';
    ...
    const needsRecalculation =
      !lastCalc ||
      new Date().getTime() - new Date(lastCalc.calculation_date).getTime() >
        30 * 24 * 60 * 60 * 1000; // 30 days

    // Generate recommendations
    if (missing.length > 0) {
      recommendations.push(
        `Complete your profile by adding: ${missing.join(', ')}`
      );
    }

    if (needsRecalculation) {
      recommendations.push(
        'Recalculate your training thresholds based on recent activities'
      );
    }

    if (completeness < 70) {
      recommendations.push(
        'Add more personal information to get better training recommendations'
      );
    }

    const lastUpdated = tp.updated_at
      ? Math.floor(
          (new Date().getTime() - new Date(tp.updated_at).getTime()) /
            (1000 * 60 * 60 * 24)
        )
      : 0;
  ...
  private static generateThresholdRecommendations(
    ...
    if (conf.max_heart_rate && conf.max_heart_rate < 0.7) {
      recommendations.push(
        'Complete more activities with heart rate data for better max HR estimation'
      );
    }

    if (
      conf.functional_threshold_power &&
      conf.functional_threshold_power < 0.7 &&
      est.functionalThresholdPower
    ) {
      recommendations.push(
        'Add more cycling activities with power data for better FTP estimation'
      );
    }

    if (conf.overall && conf.overall < 0.6) {
      recommendations.push(
        'Complete more training activities to improve threshold estimations'
      );
    }
