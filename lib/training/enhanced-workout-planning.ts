...
export class EnhancedWorkoutPlanner {
  ...
  generateWeeklyPlan(): WeeklyWorkoutPlan {
    ...
    console.log(
      'context.currentTrainingLoad:',
      this.context.currentTrainingLoad
    );
    console.log(
      'context.recentActivities.length:',
      this.context.recentActivities.length
    );
    ...
    if (Object.values(workouts).every(w => w === null)) {
      console.warn(
        'All generated workouts are null! Check input data and generation logic.'
      );
    }
  ...
  private createGoalSpecificWorkout(): EnhancedWorkoutRecommendation | null {
    ...
    const distanceGoals = activeGoals.filter(
      g => g.goal_type?.metric_type === 'total_distance'
    );
    const paceGoals = activeGoals.filter(
      g => g.goal_type?.metric_type === 'average_pace'
    );
    const frequencyGoals = activeGoals.filter(
   ...
   */
  private createDistanceGoalWorkout(
    goal: UserGoal
  ...
  private createPaceGoalWorkout(): EnhancedWorkoutRecommendation {
    return {
      ...
      recoveryTime: 48,
      reasoning:
        'To improve pace, you need structured speed work. This interval session will target your lactate threshold.',
      alternatives: [
        {
          ...
          recoveryTime: 36,
          reasoning:
            'Tempo runs improve your lactate threshold, which is key for pace improvement.',
          alternatives: [],
        },
      ],
      instructions: [
  ...
  private createFrequencyGoalWorkout(): EnhancedWorkoutRecommendation {
    return {
      ...
      recoveryTime: 12,
      reasoning:
        'To build consistency, focus on easy, enjoyable runs that you can complete regularly.',
      alternatives: [
        {
          ...
          recoveryTime: 6,
          reasoning:
            'Walking is a great way to build the habit of daily activity.',
          alternatives: [],
        },
      ],
      instructions: [
  ...
  private createIntensityWorkout(): EnhancedWorkoutRecommendation {
    ...
    if (isAdvanced) {
      return {
        ...
        recoveryTime: 48,
        reasoning:
          'Advanced interval training will push your VO2 max and improve race performance.',
        alternatives: [
          {
            ...
            recoveryTime: 36,
            reasoning:
              'Fartlek training improves both aerobic and anaerobic capacity.',
            alternatives: [],
          },
        ],
        instructions: [
      ...
      return {
        ...
        recoveryTime: 36,
        reasoning:
          'Tempo runs improve your lactate threshold, which is key for race performance.',
        alternatives: [
          {
            ...
            recoveryTime: 48,
            reasoning:
              'Threshold intervals will improve your aerobic capacity.',
            alternatives: [],
          },
        ],
        instructions: [
  ...
  private createLongWorkout(): EnhancedWorkoutRecommendation {
    ...
    return {
      ...
      recoveryTime: 48,
      reasoning:
        'Long runs build endurance and prepare you for longer races. This is the foundation of your training.',
      alternatives: [
        {
          ...
          recoveryTime: 36,
          reasoning:
            'Long cycling sessions provide excellent endurance training with less impact.',
          alternatives: [],
        },
      ],
      instructions: [
  ...
  private generateWorkoutForDay(
    ...
    switch (mondayBasedDay) {
      ...
      case 6: // Sunday - Recovery or rest
        ...
        if (
          this.context.currentTrainingLoad &&
          this.context.currentTrainingLoad.acute > 50
        ) {
  ...
  private shouldRecommendIntensityWork(): boolean {
    ...
    return (
      currentTrainingLoad &&
      currentTrainingLoad.balance > 0 &&
      currentTrainingLoad.acute < 70
    );
  }
  ...
  private shouldRecommendLongWorkout(): boolean {
    ...
    return (
      currentTrainingLoad &&
      currentTrainingLoad.balance > -10 &&
      currentTrainingLoad.chronic > 70
    );
  }
  ...
  private adjustForWeather(
    ...
    if (temperature > 25) {
      workout.weatherConsideration =
        'High temperature - reduce intensity by 1-2 points and stay hydrated';
    ...
    if (temperature < 5) {
      workout.weatherConsideration =
        'Cold weather - warm up thoroughly and dress in layers';
    ...
    if (precipitation > 0.5) {
      workout.weatherConsideration =
        'Wet conditions - reduce intensity and be extra careful on turns';
      workout.intensity = Math.max(1, workout.intensity - 1);
    }
    ...
    if (windSpeed > 20) {
      workout.weatherConsideration =
        'High winds - consider indoor alternatives or reduce intensity';
   ...
   */
  private calculateWeeklyTSS(workouts: {
    [dayOfWeek: number]: EnhancedWorkoutRecommendation | null;
  }): number {
   ...
   */
  private calculateWeeklyDistance(workouts: {
    [dayOfWeek: number]: EnhancedWorkoutRecommendation | null;
  }): number {
    return Object.values(workouts)
      .filter(workout => workout !== null)
   ...
   */
  private calculateWeeklyTime(workouts: {
    [dayOfWeek: number]: EnhancedWorkoutRecommendation | null;
  }): number {
    return Object.values(workouts)
      .filter(workout => workout !== null)
