...
export class TrainingProfileService {
  ...
  static async getCompleteProfile(
    ...
    try {
      ...
      return {
        ...
        preferences:
          (preferences as TrainingPreferences) ||
  ...
  static async calculateThresholds(userId: string): Promise<unknown> {
    try {
      ...
      const { TrainingZoneAnalysis } = await import(
        '@/lib/training/zone-analysis'
      );
  ...
  static analyzeProfile(profile: CompleteUserProfile): ProfileAnalysis {
    ...
    if (userProfile.age) completeness += 8;
    else missing.push('age');
    if (userProfile.weight) completeness += 8;
    else missing.push('weight');
    if (userProfile.sex) completeness += 8;
    ...
    if (preferences.primary_goal) completeness += 12;
    else missing.push('primary goal');
    if (preferences.goal_description) completeness += 8;
    else missing.push('goal description');
    if (preferences.preferred_training_days.length > 0) completeness += 10;
    else missing.push('training schedule');
    if (preferences.max_weekly_training_time) completeness += 8;
    else missing.push('weekly training time');
    ...
    if (
      ...
      missing.includes('biological sex')
    ) {
      recommendations.push(
        'Add basic physical info to improve TSS target calculation'
      );
    }

    if (missing.includes('primary goal')) {
      recommendations.push(
        'Set a primary training goal to get personalized workout suggestions'
      );
    }

    if (missing.includes('training schedule')) {
      recommendations.push(
        'Define your preferred training days for better workout planning'
      );
    }

    if (completeness < 70) {
      recommendations.push(
        'Complete your profile to unlock personalized training insights'
      );
    }

    // Calculate personalized TSS target
    const personalizedTSSTarget = this.calculatePersonalizedTSSTarget(
      userProfile,
      preferences
    );
