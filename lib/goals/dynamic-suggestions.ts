...
export class DynamicGoalEngine {
  static analyzeUserPerformance(
    ...
    const weeklyDistance =
      recentActivities.length > 0
        ? recentActivities.reduce((sum, a) => sum + (a.distance || 0), 0) /
          1000 /
          4.3 // Convert to km/week
        : 0;

    const monthlyDistance =
      recentActivities.reduce((sum, a) => sum + (a.distance || 0), 0) / 1000;
    ...
    const averagePace =
      runActivities.length > 0
        ? runActivities.reduce(
            (sum, a) => sum + (a.moving_time || 0) / ((a.distance || 1) / 1000),
            0
    ...
    const longestRun = Math.max(
      ...runActivities.map(a => (a.distance || 0) / 1000),
      0
    );
    ...
    const averageHeartRate = recentActivities
      ...
      .reduce(
        (sum, a, _, arr) => sum + (a.average_heartrate || 0) / arr.length,
        0
      );
    ...
    // Activity preferences
    const sportTypeCounts = recentActivities.reduce(
      (acc, a) => {
    ...
    const dayOfWeekCounts = recentActivities.reduce(
      (acc, a) => {
    ...
    const averageActivityDuration =
      recentActivities.length > 0
        ? recentActivities.reduce((sum, a) => sum + (a.moving_time || 0), 0) /
          recentActivities.length /
          60
        : 0;
    ...
    const runningExperience = this.determineExperienceLevel(
      totalActivities,
      weeklyDistance
    );
  ...
  static generateDynamicSuggestions(
    ...
    if (
      ...
      !activeGoalCategories.includes('distance')
    ) {
    ...
    if (
      ...
      !activeGoalCategories.includes('pace')
    ) {
    ...
    if (
      ...
      !activeGoalCategories.includes('frequency')
    ) {
    ...
    suggestions.push(
      ...this.suggestExperienceBasedGoals(profile, unitPreferences)
    );

    // 6. Additional suggestions for beginners with some progress
    if (
      profile.runningExperience === 'beginner' &&
      profile.weeklyDistance >= 10
    ) {
      suggestions.push({
        id: 'dynamic-beginner-progression',
        title: 'Take the Next Step',
        description:
          'Add variety to your training with different types of runs',
        reasoning:
          "You've built a solid foundation. Now let's add some variety to keep things interesting.",
        priority: 'medium',
        category: 'frequency',
  ...
  private static determineExperienceLevel(
    ...
    if (
      totalActivities < BEGINNER_ACTIVITY_THRESHOLD ||
      weeklyDistance < BEGINNER_DISTANCE_THRESHOLD
    )
      return 'beginner';
    if (
      totalActivities < INTERMEDIATE_ACTIVITY_THRESHOLD ||
      weeklyDistance < INTERMEDIATE_DISTANCE_THRESHOLD
    )
      return 'intermediate';
    return 'advanced';
  }

  private static suggestDistanceGoal(
    profile: UserPerformanceProfile,
    unitPreferences?: { distance: 'km' | 'miles'; pace: 'min/km' | 'min/mile' }
  ): DynamicGoalSuggestion {
    ...
    return {
      ...
      reasoning: `You've been consistently improving your distance. Your current average of ${
        unitPreferences?.distance === 'miles'
          ? (currentWeekly * 0.621371).toFixed(1)
          : currentWeekly.toFixed(1)
      }${unitPreferences?.distance === 'miles' ? 'mi' : 'km'}/week shows you're ready for the next challenge.`,
      priority: 'high',
      category: 'distance',
  ...
  private static suggestPaceGoal(
    profile: UserPerformanceProfile,
    unitPreferences?: { distance: 'km' | 'miles'; pace: 'min/km' | 'min/mile' }
  ): DynamicGoalSuggestion {
  ...
  private static suggestConsistencyGoal(
    profile: UserPerformanceProfile
  ): DynamicGoalSuggestion {
  ...
  private static suggestChallengeGoals(
    profile: UserPerformanceProfile,
    ...
    if (profile.longestRun < 10 && !activeCategories.includes('challenge')) {
      ...
      suggestions.push({
        ...
        reasoning: `Your longest run is ${
          unitPreferences?.distance === 'miles'
            ? (profile.longestRun * 0.621371).toFixed(1)
            : profile.longestRun.toFixed(1)
        }${unitPreferences?.distance === 'miles' ? 'mi' : 'km'}. A ${challengeDistance}${distanceUnit === 'miles' ? 'mi' : 'km'} is a perfect next milestone.`,
        priority: 'medium',
        category: 'distance',
        goalType: this.mapCategoryToGoalType('distance'),
  ...
  private static suggestFrequencyRecoveryGoal(
    profile: UserPerformanceProfile
  ): DynamicGoalSuggestion {
    return {
      ...
      reasoning:
        "Your running frequency has decreased recently. Let's rebuild that habit.",
      priority: 'high',
      category: 'frequency',
      goalType: this.mapCategoryToGoalType('frequency'),
  ...
  private static suggestSpeedWorkGoal(
    profile: UserPerformanceProfile,
    unitPreferences?: { distance: 'km' | 'miles'; pace: 'min/km' | 'min/mile' }
  ): DynamicGoalSuggestion {
    ...
    return {
      ...
      reasoning:
        'Your pace has been slowing. Strategic speed work can help reverse this trend.',
      priority: 'medium',
      category: 'pace',
      goalType: this.mapCategoryToGoalType('pace'),
  ...
  private static suggestExperienceBasedGoals(
    profile: UserPerformanceProfile,
    unitPreferences?: { distance: 'km' | 'miles'; pace: 'min/km' | 'min/mile' }
  ): DynamicGoalSuggestion[] {
    const suggestions: DynamicGoalSuggestion[] = [];
    ...
    if (profile.runningExperience === 'beginner') {
      ...
      suggestions.push({
        ...
        reasoning:
          'As a beginning runner, consistency is more important than intensity.',
        priority: 'high',
        category: 'frequency',
        goalType: this.mapCategoryToGoalType('frequency'),
        suggestedTarget: 3,
      ...
      if (profile.weeklyDistance > 0) {
        ...
        if (unitPreferences?.distance === 'miles') {
          suggestedDistance =
            Math.round(suggestedDistance * 0.621371 * 10) / 10;
          distanceUnit = 'miles';
        }
        ...
        suggestions.push({
          ...
          reasoning: `You're currently running ${
            unitPreferences?.distance === 'miles'
              ? (profile.weeklyDistance * 0.621371).toFixed(1)
              : profile.weeklyDistance.toFixed(1)
          }${unitPreferences?.distance === 'miles' ? 'mi' : 'km'} per week. Let's build on that foundation.`,
          priority: 'medium',
          category: 'distance',
          goalType: this.mapCategoryToGoalType('distance'),
      ...
      if (profile.averageActivityDuration > 0) {
        const suggestedDuration = Math.round(
          profile.averageActivityDuration * 1.3
        );
      ...
      if (profile.runFrequency >= 2 && profile.weeklyDistance >= 5) {
        ...
        suggestions.push({
          ...
          reasoning:
            "You've built good consistency. Now let's work on maintaining a steady pace.",
          priority: 'low',
          category: 'pace',
          goalType: this.mapCategoryToGoalType('pace'),
          suggestedTarget: suggestedPace,
  ...
  private static mapCategoryToGoalType(category: string): GoalType {
    ...
    const goalTypeMap: Record<string, GoalType> = {
      distance: {
        ...
        unit: 'km',
        target_guidance:
          'Beginners: 15-25km, Intermediate: 30-50km, Advanced: 60km+',
      ...
      pace: {
        ...
        unit: 'min/km',
        target_guidance:
          'Aim to improve by 10-20 seconds per km over 2-3 months',
      ...
      frequency: {
        ...
        unit: 'runs',
        target_guidance:
          'Beginners: 3 runs, Intermediate: 4-5 runs, Advanced: 6+ runs',
      ...
      duration: {
        ...
        unit: 'hours',
        target_guidance:
          'Beginners: 2-4 hours, Intermediate: 5-8 hours, Advanced: 10+ hours',
