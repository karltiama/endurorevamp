...
export class BackgroundSyncService {
   ...
   */
  static async runBackgroundSync(options: BackgroundSyncOptions = {}): Promise<SyncStats> {
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          const result = await this.syncUserActivities(user.user_id, opts.syncType)
          ...
          if (result.success) {
            ...
            stats.failedSyncs++
            stats.errors.push(`User ${user.user_id}: ${result.error || 'Unknown error'}`)
          }
        } catch (error) {
          stats.failedSyncs++
  ...
  private static async syncUserActivities(userId: string, syncType: string) {
    try {
      ...
      if (result.success) {
        console.log(`✅ Sync successful for user ${userId}: ${result.activitiesSynced} activities`)
  ...
  static async getSyncStatistics() {
    ...
    return {
      ...
      recentSyncs: stats.filter(s => 
        s.last_sync_at && new Date(s.last_sync_at).getTime() > oneHourAgo
      ).length,
      staleSyncs: stats.filter(s => 
        !s.last_sync_at || new Date(s.last_sync_at).getTime() < oneDayAgo
      ).length,
      expiredTokens: stats.filter(s => 
