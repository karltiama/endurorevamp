...
export function convertPace(secondsPerKm: number, toUnit: PaceUnit): number {
  return toUnit === 'min/mile'
    ? secondsPerKm * CONVERSION_CONSTANTS.MILES_TO_KM
    : secondsPerKm;
}
...
export function convertTemperature(
  ...
  return toUnit === 'fahrenheit'
    ? CONVERSION_CONSTANTS.CELSIUS_TO_FAHRENHEIT(celsius)
    : celsius;
}
...
export function parseHevyWorkout(description: string): {
  ...
  for (const line of lines) {
    ...
    const setMatch = trimmedLine.match(
      /Set \d+:\s*(\d+(?:\.\d+)?)\s*(?:lbs?|kg)\s*x\s*(\d+)/i
    );
 ...
 */
export function formatHevyWorkout(
  parsedWorkout: ReturnType<typeof parseHevyWorkout>
): string {
