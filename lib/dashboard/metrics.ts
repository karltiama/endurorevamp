...
export function calculateWeeklyDistance(activities: Activity[]): {
  current: number;
  previous: number;
  change: number;
} {
  ...
  const currentDistance = currentWeekActivities.reduce(
    (sum, activity) => sum + (activity.distance || 0),
    0
  );
  const previousDistance = previousWeekActivities.reduce(
    (sum, activity) => sum + (activity.distance || 0),
    0
  );
...
export function calculateActivityStreak(activities: Activity[]): {
  current: number;
  longest: number;
  consistency: number;
} {
  ...
  const recentActivityDates = activityDates.filter(
...
export function calculateMonthlyProgress(activities: Activity[]): {
  ...
  const currentDistance = monthActivities.reduce(
    (sum, activity) => sum + (activity.distance || 0),
    0
  );
  ...
  if (hasEnoughData) {
    ...
    const daysSinceStart = Math.max(
      1,
      Math.floor(
        (now.getTime() - new Date(oldestActivity.start_date).getTime()) /
          (1000 * 60 * 60 * 24)
      )
    );
