...
export class WeatherService {
   ...
   */
  getOptimalRunningTime(forecast: WeatherData): { time: string; reason: string } {
  ...
  private analyzeTemperature(weather: RunningWeatherConditions, impact: WeatherImpact): void {
    ...
    if (temperature < 0) {
      ...
      impact.adjustments.clothing.push('Thermal base layer', 'Windproof jacket', 'Gloves', 'Hat')
      ...
      impact.adjustments.hydration.push('Drink 500ml before run', 'Carry water')
    } else if (temperature > 25) {
      impact.performance = 'negative'
      impact.risk = 'high'
      impact.recommendations.push('High heat - reduce intensity, stay hydrated')
      impact.adjustments.intensity -= 2
      impact.adjustments.duration -= 20
      impact.adjustments.hydration.push('Electrolyte replacement', 'Frequent water breaks')
    }
  }
  ...
  private analyzeHumidity(weather: RunningWeatherConditions, impact: WeatherImpact): void {
    ...
    if (humidity > 80) {
      impact.performance = 'negative'
      impact.recommendations.push('High humidity - reduce intensity, stay hydrated')
      impact.adjustments.intensity -= 1
  ...
  private analyzeWind(weather: RunningWeatherConditions, impact: WeatherImpact): void {
    ...
    if (windSpeed > 25) {
      ...
      impact.adjustments.route.push('Headwind on way out, tailwind on way back')
    }
  }
   ...
   */
  private analyzePrecipitation(weather: RunningWeatherConditions, impact: WeatherImpact): void {
   ...
   */
  private analyzeEnvironmentalFactors(weather: RunningWeatherConditions, impact: WeatherImpact): void {
    ...
    if (airQuality > 100) {
      impact.performance = 'negative'
      impact.risk = 'high'
      impact.recommendations.push('Poor air quality - consider indoor alternatives')
      impact.adjustments.intensity -= 1
    }
  }
   ...
   */
  private getRunningTimeWindows(): Array<{ name: string; startHour: number; endHour: number; priority: number }> {
    return [
  ...
  private calculateRealisticRunningScore(hour: {
    ...
    if (windowName.includes('early_morning') || windowName.includes('evening')) {
      if (hour.uvIndex <= 3) {
